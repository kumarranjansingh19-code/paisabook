...
describe('parseAlert', () => {
  ...
  it('reads a Federal NEFT salary credit', () => {
    const a = parseAlert(mail('Federal Bank <alerts@federalbank.co.in>', 'Credit alert', 'Rs 2,75,000.00 credited to your A/c XX4488 on 01-08-2026 by NEFT from ACME SOFTWARE PVT LTD. Ref no N213261234567.'));
    expect(a).toMatchObject({ amount: '2,75,000.00', direction: 'credit', date: '2026-08-01', account_kind: 'bank', ref_no: 'N213261234567' });
    expect(a!.narration).toContain('ACME SOFTWARE');
  });
  ...
  it('reads card payment receipts as credits and skips FD renewals', () => {
    const yes = parseAlert(mail('YES BANK <alerts@custcom.yes.bank.in>', 'Payment received', 'Dear Customer, we have received the payment of Rs. 6,210.00 towards your YES BANK Credit Card XX8801 on 20-08-2026. Thank you for your payment.'));
    expect(yes).toMatchObject({ direction: 'credit', amount: '6,210.00', account_kind: 'credit_card' });
    const hdfc = parseAlert(mail('HDFC Bank <alerts@hdfcbank.net>', 'Payment credited', 'Payment of Rs 18,420.00 has been credited to your HDFC Bank Credit Card ending 6042 on 17-08-2026 via CRED. Available limit Rs 1,50,000.'));
    expect(hdfc).toMatchObject({ direction: 'credit', amount: '18,420.00' });
  ...
  it('reads real SBI CBS alerts (long masks, "has a debit by", text after the date)', () => {
    const nach = parseAlert(mail('<cbsalerts.sbi@alerts.sbi.bank.in>', 'CBSSBI ALERT', 'Greetings from SBI ! Dear Customer, Your A/C XXXXX097310 has a debit by NACH of Rs 1,000.00 on 12/08/26. Avl Bal Rs 1,09,591.35. Download YONO - SBI. Please do not reply to this auto generated email.'));
    expect(nach).toMatchObject({ direction: 'debit', amount: '1,000.00', date: '2026-08-12' });
    expect(nach!.account_hint).toBe('SBI A/c XX7310');
    expect(nach!.narration).toMatch(/NACH/);
    const ecs = parseAlert(mail('<cbsalerts.sbi@alerts.sbi.bank.in>', 'CBSSBI ALERT', 'Greetings from SBI ! Your AC XXXXX097310 Debited INR 295.00 on 03/08/26 -ECS/ACH RET CH. Avl Bal INR 18.35.-SBI Please do not reply.'));
    expect(ecs).toMatchObject({ direction: 'debit', amount: '295.00', date: '2026-08-03' });
    expect(ecs!.narration).toMatch(/ECS\/ACH RET CH/);
    const neft = parseAlert(mail('"neftinfo.itps" <neftinfo.itps@alerts.sbi.bank.in>', 'NEFT Transaction', 'Dear Customer, Thank you for banking with State Bank of India. Your account has been credited for NEFT received as per the details given below Credited to Your A/c: XX7310 Amount: INR 1,10,000.00 UTR No.: FBBT262100004321 Date: 04/08/2026 Sent by: Asha Verma Sender Bank IFSC: FDRL0001234'));
    expect(neft).toMatchObject({ direction: 'credit', amount: '1,10,000.00', date: '2026-08-04', ref_no: 'FBBT262100004321' });
    expect(neft!.narration).toBe('Asha Verma');
  });
  it('reads a CRED bill-payment confirmation as a credit on the card', () => {
    const a = parseAlert({ ...mail('CRED <protect@cred.club>', 'your credit card bill payment was successful', 'hey, your credit card bill payment was successful. ₹6,210.00 paid to YES Bank •••• 8801 on 20 Aug 2026. ref YDZJ6AB1X3R. you earned 649 CRED coins.'), receivedAt: '2026-08-20T09:00:00.000Z' });
    expect(a).toMatchObject({ direction: 'credit', amount: '6,210.00', account_kind: 'credit_card', date: '2026-08-20' });
  ...
  it('reads an SBI IMPS debit where the beneficiary is also mentioned', () => {
    const a = parseAlert(mail('SBI <alerts@sbi.co.in>', 'Alert', 'Your A/c X7310 is debited for Rs 2,500.00 on 12/08/26 and A/c XX9999 credited (IMPS Ref no 123456789012).'));
    expect(a).toMatchObject({ direction: 'debit', amount: '2,500.00', date: '2026-08-12', ref_no: '123456789012' });
    expect(a!.account_hint).toBe('SBI A/c XX7310');
  });
});
...
describe('detectStatement / parseBill', () => {
  ...
  it('reads a bill notice', () => {
    const b = parseBill({ from: 'SBI Card <statements@sbicard.com>', subject: 'Your SBI Card statement is ready', bodyText: 'Statement date 05 Aug 2026. Card ending 3375. Total Amount Due: Rs 25,999.00. Minimum Amount Due Rs 1,300.00. Payment Due Date: 25 Aug 2026.' });
    expect(b).toMatchObject({ total_due: '25,999.00', min_due: '1,300.00', due_date: '2026-08-25', statement_date: '2026-08-05' });
    expect(b!.card_hint).toBe('SBI Card Credit Card XX3375');
  });
});
...
describe('discoverHeuristically', () => {
  it('groups by institution, kind and last4', () => {
    const r = discoverHeuristically([
      ...
      { from: 'cc.statements@axisbank.com', subject: 'Credit Card Statement', snippet: 'card XX9127 statement attached' },
      { from: 'news@shop.com', subject: 'Sale', snippet: 'Rs 499 only' },
    ]);
    expect(r.proposals.map((p) => `${p.institution}|${p.kind}|${p.last4}|${p.seen}`)).toEqual(['HDFC Bank|bank|1234|2', 'Axis Bank|credit_card|9127|1']);
  ...
  it('ignores marketing, bank names inside bodies, and bare 4-digit numbers', () => {
    const r = discoverHeuristically([
      ...
      { from: 'alerts@sbi.co.in', subject: 'CBSSBI ALERT', snippet: 'Your A/c X7310 is credited Rs 90.00 on 12/08/26 by RADICO' },
      { from: 'alerts@sbi.co.in', subject: 'NEFT Transaction', snippet: 'Your A/c X7310 is credited Rs 1,10,000 by NEFT' },
    ]);
    expect(r.proposals.map((p) => `${p.institution}|${p.kind}|${p.last4}`)).toEqual(['SBI|bank|7310']);
  });
});
